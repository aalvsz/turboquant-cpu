            ...
            case LLM_ARCH_GEMMA4:
                {
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        const int64_t n_embd_head_i = hparams.n_embd_head_k_layer(i);
                        const int64_t n_embd_k_i    = hparams.n_embd_k_gqa(i);
                        const int64_t n_embd_v_i    = hparams.n_embd_v_gqa(i);
                        const int     kv_flags      = hparams.has_kv(i) ? 0 : TENSOR_NOT_REQUIRED;

                        if (n_embd_head_i != (int64_t) hparams.n_embd_head_v_layer(i)) {
                            throw std::runtime_error("Gemma 4 requires per-layer n_embd_head_k == n_embd_head_v");
                        }

                        layer.attn_norm = create_tensor(tn(LLM_TENSOR_ATTN_NORM, "weight", i), {n_embd}, 0);

                        layer.wq = create_tensor(tn(LLM_TENSOR_ATTN_Q,   "weight", i), {n_embd, n_embd_head_i * n_head_i}, 0);
                        layer.wk = create_tensor(tn(LLM_TENSOR_ATTN_K,   "weight", i), {n_embd, n_embd_k_i}, kv_flags);
                        layer.wv = create_tensor(tn(LLM_TENSOR_ATTN_V,   "weight", i), {n_embd, n_embd_v_i}, TENSOR_NOT_REQUIRED);
                        ...
                        layer.attn_k_norm    = create_tensor(tn(LLM_TENSOR_ATTN_K_NORM,    "weight", i), {n_embd_head_i}, kv_flags);
                        layer.attn_post_norm = create_tensor(tn(LLM_TENSOR_ATTN_POST_NORM, "weight", i), {n_embd}, 0);
